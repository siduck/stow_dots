...
pin_app_toggle() {
	clicked_win=$(eww get clicked_win)

	if [ "$clicked_win" = "$1_on" ]; then
		eww update clicked_win="$1_off"
	else
		eww update clicked_win="$1_on"
	fi
}
...
toggle_volume() {
	...
	eww update volume_slider="$new_val"
}
